...
| `BB_TRANSCRIPTION`             | `bb-app config`                                    | Optional                | Voice transcription model in `<service>/<model>` format: a plugin-registered AI service (`codex` with the codex plugin, or `whisper` with the Local voice plugin; audio up to 5MB) or `openai/<model>` with `OPENAI_API_KEY`. Defaults to `codex/gpt-transcribe`.                                                                                                                                                                                                                                                                                                                                    |
...
| `BB_API_TOKEN`                 | environment                                        | CLI/SDK only            | Overrides the local API token the `bb` CLI and Node SDK send as a bearer header. Defaults to the contents of `api-token` in the data dir, which the server creates on first start. Every `/api/v1` request and `/ws` socket must present the token; browsers receive it as an HttpOnly cookie from `/auth/session?token=…`. A request that presents the token as a bearer header skips the browser `Origin` check, so browser extensions and other non-app clients that hold the token can call `/api/v1` from any origin; cookie-authenticated requests still must come from a local BB app origin. |
...
Without any OpenAI or Codex account, the built-in Local voice plugin
transcribes on the primary host with whisper.cpp. Install `whisper.cpp` and
`ffmpeg` on that host, run `bb whisper prepare base.en`, then set
`BB_TRANSCRIPTION` to `whisper/base.en`. `bb whisper status` shows the
resolved tools and downloaded models.
