...
| `BB_API_TOKEN`                 | environment                                        | CLI/SDK only            | Overrides the local API token the `bb` CLI and Node SDK send as a bearer header. Defaults to the contents of `api-token` in the data dir, which the server creates on first start. Every `/api/v1` request and `/ws` socket must present the token; browsers receive it as an HttpOnly cookie from `/auth/session?token=…`.                                                                                    |
