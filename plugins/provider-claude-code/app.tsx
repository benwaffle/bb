...
import type { claudeSessionImportRpcContract } from "./src/session-import-rpc.js";
...
function ClaudeSessionImportSection({ projectId }: PluginHomepageSectionProps) {
  ...
  useEffect(() => {
    if (expanded && listing === null && !loading && error === null) {
      void load(null);
    }
  }, [expanded, listing, loading, error, load]);
  ...
  const visible = useMemo(() => {
    ...
    return sortForProject(
      listing.sessions.filter((session) => matchesFilter(session, needle)),
      projectId,
    );
  }, [listing, filter, projectId]);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {loading && listing === null ? (
        <div className="text-muted-foreground text-xs">Reading sessions…</div>
      ) : null}
      ...
      <div className="text-muted-foreground text-xs">
        Importing creates a thread with the session's turns. Its first message
        forks the Claude session from the last imported turn, so the terminal
        session is never modified. A session whose directory belongs to a bb
        project runs there; any other session becomes a personal thread unless
        you pick a project on its row, in which case it runs in that project's
        default environment.
      </div>
    </div>
  );
